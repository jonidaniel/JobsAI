 ...
 *
 * @param formData - The form data object containing all question responses.
 *   Keys match GENERAL_QUESTION_KEYS and "additional-info".
 *
 * @returns Validation result object:
 *   - isValid: True if all mandatory questions are valid
 *   - errors: Map of field keys to error messages.
 ...
 * }
 */
export function validateGeneralQuestions(formData: FormData): ValidationResult {
  ...
  if (!jobLevel || !Array.isArray(jobLevel) || jobLevel.length === 0) {
    ...
    const index1 = NAME_OPTIONS.indexOf(jobLevel[0] as string);
    const index2 = NAME_OPTIONS.indexOf(jobLevel[1] as string);
  ...
  if (!deepMode || (typeof deepMode === "string" && deepMode.trim() === "")) {
  ...
  if (
    !coverLetterNum ||
    (typeof coverLetterNum === "string" && coverLetterNum.trim() === "")
  ) {
